                ...
                <form action="" method="post" enctype='multipart/form-data' >
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <select id="category_id" class="form-control" name="category_id">
                                ...
                                <?php foreach($categories as $category){ ?>
                                    <option value="<?php echo $category->id; ?>" <?php if(!empty($record->category_id) && $record->category_id==$category->id){ echo "selected"; } ?>><?php echo $category->category_title; ?></option>
                                <?php } ?>
                            </select>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="hidden" id="edit-product-id" value="<?php echo $record->id; ?>">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="product_type" id="product_type" class="form-control">
                                <option value="piece" <?php if(!empty($record->product_type) && $record->product_type=='piece'){ echo "selected"; } ?>>Piece</option>
                                <option value="meter" <?php if(!empty($record->product_type) && $record->product_type=='meter'){ echo "selected"; } ?>>Meter</option>
                                <option value="ft" <?php if(!empty($record->product_type) && $record->product_type=='ft'){ echo "selected"; } ?>>Ft</option>
                                <option value="cm" <?php if(!empty($record->product_type) && $record->product_type=='cm'){ echo "selected"; } ?>>Cm</option>
                                <option value="inch" <?php if(!empty($record->product_type) && $record->product_type=='inch'){ echo "selected"; } ?>>Inch</option>
                                <option value="liter" <?php if(!empty($record->product_type) && $record->product_type=='liter'){ echo "selected"; } ?>>Liter</option>
                                <option value="ml" <?php if(!empty($record->product_type) && $record->product_type=='ml'){ echo "selected"; } ?>>Ml</option>
                                <option value="dozen" <?php if(!empty($record->product_type) && $record->product_type=='dozen'){ echo "selected"; } ?>>Dozen</option>
                                <option value="kg" <?php if(!empty($record->product_type) && $record->product_type=='kg'){ echo "selected"; } ?>>Kg</option>
                                <option value="gm" <?php if(!empty($record->product_type) && $record->product_type=='gm'){ echo "selected"; } ?>>Gm</option>
                            </select>
                          ...
                          <div class="form-group col-md-6">
                            ...
                            <select id="brand_id" class="form-control" name="brand_id">
                                ...
                                <?php foreach($brands as $brand){ ?>
                                    <option value="<?php echo $brand->id; ?>" <?php if(!empty($record->brand_id) && $record->brand_id==$brand->id){ echo "selected"; } ?>><?php echo $brand->brand_name; ?></option>
                                <?php } ?>
                            </select>
                    ...
                    <div class="form-row">
                        <div class="form-group col-sm-6">
                            ...
                            <div id="color_field">
                                <?php if(!empty($record->product_color)){
                                    $explode_color=explode(',',$record->product_color);
                                    foreach($explode_color as $color){
                                ?>
                                    <div class="row mt-2">
                                        <div class="col-md-10">
                                            <input type="color"  name="product_color[]" value="<?php echo $color; ?>" class="d-inline form-control"/>
                                        </div>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <div id="highlight_field">
                                <?php if(!empty($record->product_highlight)){
                                    $explode_highlight=explode(',',$record->product_highlight);
                                    foreach($explode_highlight as $highlight){
                                ?>
                                    <div class="row mt-2">
                                        <div class="col-md-10">
                                            <input type="text"  name="product_highlight[]" value="<?php echo $highlight; ?>" class="d-inline form-control"/>
                                        </div>
                                        <button class="d-inline btn btn-danger" onclick="this.parentNode.parentNode.removeChild(this.parentNode)">Del</button>
                                    </div>
                                <?php }} ?>
                        ...
                        <div class="form-group col-sm-6">
                         ...
                         <div class="form-group col-md-6">
                            ...
                            <select  id="multiple" class="js-states form-control" name="similar_product_id[]" multiple>
                                ...
                                <?php foreach($similar_product as $key=>$product){ ?>
                                
                                    <option value="<?php echo $product->id; ?>" <?php if(!empty($record->similar_product_id) &&  in_array($product->id,$record->similar_product_id)){ echo "selected"; } ?>><?php echo $product->product_name; ?></option>
                                <?php } ?>
                            </select>
                    ...
                    <?php if(!empty($gallery)){ ?>
                        ...
                        <div class="form-row mb-2">
                            <?php  foreach($gallery as $gimage){ ?>
                                <div class="img-wrap ml-2" id="gallery-<?php echo $gimage->id; ?>">
                                    <button type="button" class="clear" onclick="deleteGalleryModel(`<?php echo $gimage->id; ?>`,`<?php echo $gimage->image ?>`)" class="hide">x</button>
                                    <img id="output" src="<?php echo public_path('product_gallery/'.$gimage->image); ?>" style="max-height:197px;"> 
                                </div>
                            <?php } ?>
                        </div>
                    <?php } ?>
...
<div class="modal fade" id="delete-gallery-model" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="delete-gallery-model" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">Delete</h5>
        </div>
        <div class="modal-body text-center" id="delete-model-progress">
