...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mb-3 ">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        ...
        <a href="<?php echo site_url('user/userEdit') ?>" class="btn btn-sm btn-primary ml-2"> Add New User</a>
    </div>
</div>
                                    ...
                                    foreach($users as $row){
                                        ?>
                                            <tr>
                                                ...
                                                <td>
                                                <img title="<?php echo $row->first_name." ".$row->last_name; ?> " src="<?php echo GetQrCode($row->phone); ?>" onmouseover="onhoverchange(this)" data-toggle="modal" data-target="#qrcodeModel" style="width:50px;height:50px;" >
                                                </td>
                                                <td>
                                                    <strong><?php echo $row->first_name." ".$row->last_name; ?>    </strong>                                            
                                                    <p class="m-0"><?php echo $row->phone; ?></p>
                                                   <p class="m-0"> <?php echo $row->email; ?></p>
                                                   <p class="m-0">ID-  <?php echo $row->id; ?></p>
                                                </td>
                                                <td>
                                                ...
                                                <br>
                                                <a target="_blank"  href="<?php echo 'https://texcutive.com/home/access_account/'.urlencode(rand(10000000,99999999).'-'.$row->phone."-".rand(10000,99999999)); ?>" ><span class="fa fa-lock"> Access Account<span></a>
                                               
                                                </td>
                                                <td> <a class="btn btn-primary" href="<?php echo site_url('user/stock/'.$row->id) ?>" >Stock</a>
                                                </td>
                                                <td>
...
<!-- Modal -->
<div class="modal fade" id="qrcodeModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">QR CODE</h5>
         <a class="btn btn-primary btn-sm ml-2" href="#" id="downloadqrcode" download > Download</a>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="row"><img src="" id="imgid" style="width: 100%;" ></div>
      </div>
...
<script>
    function onhoverchange(vals){
        document.getElementById("imgid").src=vals.src;
        document.getElementById("exampleModalLabel").innerHTML =vals.title;
        document.getElementById("downloadqrcode").setAttribute("href", vals.src);
       // console.log("hi");
...
function GetQrCode($phone)
    {
			...
			$mydata=[];
			$data=['phone'=>$phone,'type'=>"wallet"];
			$userinfo=urlencode(base64_encode(json_encode($data)));	
			
			$filename = $phone.'_qrcode.png';			
			$errorCorrectionLevel = 'L'; 
			$matrixPointSize = 15; 
			QRcode::png($userinfo, "/www/wwwroot/admin.texcutive.com/qrcodes/".$filename, $errorCorrectionLevel, $matrixPointSize,2); 
			
			return "https://admin.texcutive.com//qrcodes/".$filename;
